...
public class GetAdministrativo extends UseCase<GetAdministrativo.RequestValues, GetAdministrativo.ResponseValues> {

    private ContactosRepository contactosRepository;

    public GetAdministrativo(ContactosRepository contactosRepository) {
        this.contactosRepository = contactosRepository;
    }

    @Override
    protected void executeUseCase(GetAdministrativo.RequestValues requestValues) {
        contactosRepository.getDirectivos(requestValues.getIdAlumno(), requestValues.getIdProgramaEducativo(), new ContactosDataSource.SucessCallback<List<Object>>() {
            @Override
